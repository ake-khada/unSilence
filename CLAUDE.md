...
**Last updated:** May 7, 2026 (P13: per-author FeedRow cache key, sampled signal trigger, control-plane channel batching, merge-sort `_events`, viewport-driven warm-zone hydration via CardHydrator.)
...
- **TimelineConsumer** — sole feed-state primitive (FeedViewModel, ProfileViewModel, UserProfileViewModel all delegate to instances). Owns `_events: MutableStateFlow<List<NostrEvent>>`, `_pendingNew`, `_isAtTop`, `_isLoading`. Subscribes via `TimelineService.subscribeTimeline` and routes incoming relay batches through `mergeSorted(current, newEvents)` — dedup via HashSet, sort only the novel events, two-pointer merge into the existing sorted list. Replaces an earlier full `(new+old).distinctBy.sortedWith` quicksort that ran on every relay batch (30+ full sorts per Following warm-up). Append-mode growth: live tail-in at top when `_isAtTop`, blue-dot pending buffer when scrolled, `loadMore()` extends at bottom. `feedRows: StateFlow<List<FeedRow>>` derived ONLY from `_events × _contentFilter` plus a 250ms-sampled `_statsSignal` trigger — profile/action signals are NOT in the trigger because per-card observation handles them (see EventCard). New posts tail in immediately; profile updates re-compose only the affected cards.
...
46. **TimelineConsumer.feedRows trigger excludes profile/action signals** — derived from `_events × _contentFilter` plus 250ms-sampled `_statsSignal`. Profile reactivity is per-card (EventCard observes `profileFlow(model.pubkey)` directly). Adding profile/action signals back to the trigger reintroduces list-wide rebuilds on every kind-0 arrival
