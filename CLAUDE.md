...
**Last updated:** April 6, 2026 (notification filter, AMOLED grey tones, ArticleCard timestamp, bridge repost profiles, avatar tap, crosspost media, perf polish)
...
**FeedHydrationController** — Single zone-aware scroll state machine replacing 5 independent fetch systems (engagement channel, CardHydrator debounce, ProfileResolver scroll path, loadMore trigger, FeedStateReducer velocity awareness). 4-state machine: WARM_CATCHUP (profiles only for visible items, 3s timeout gate), SLOW_SCROLL (profiles first + refs for warm zone next 15 items), IDLE (profiles + refs + engagement network fetch + 30s stale refresh), FAST_SCROLL (total blackout, zero new requests). Two-phase hydration priority: Phase 1 (profiles/identity) always fires before Phase 2 (refs/thumbnails) — avatars appear before engagement counts. Separate tracking sets (`profileHydratedIds`, `refHydratedIds`) and jobs (`profileJob`, `refJob`). Velocity detection via 6-frame sliding window with hysteresis (enter FAST >2500px/s, exit <1200px/s). 200ms minimum hold time per state prevents oscillation at velocity boundaries (IDLE exempt — has own 500ms timer). All transitions routed through `transitionTo()`. Constructed in FeedViewModel with viewModelScope, not Hilt-injected. Fed every frame from FeedScreen snapshotFlow. Calls CardHydrator as stateless worker. Phase 2 debounce: 500ms minimum interval between ref hydration runs — prevents tiny repeated passes during scroll settle. FAST_SCROLL entry cancels all in-flight hydration jobs.
