...
**Last updated:** April 7, 2026 (hydration idempotency P0-P4: window dedup, emission log dedup, indexer relay skip, NIP-19 cache, closeable audit)
...
| relay_trust_scores | Relay trust scores from trustedrelays.xyz (kind 30385 / HTTP fallback) |
...
- Relay trust scores: colored dots (green ≥70, yellow 40-69, red <40, gray no data) on all relay rows; tap opens detail bottom sheet with reliability/quality/accessibility progress bars, confidence, policy, region, operator verification. Data from trustedrelays.xyz kind 30385 events (Nostr-native) with HTTP API fallback. 24h cache via `relay_trust_scores.updated_at`.
...
- (none — hydration idempotency shipped)
