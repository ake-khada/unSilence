...
**Last updated:** April 10, 2026 (UX polish sprint: snackbar, share, zap flow fix + persistence, settings stubs, thread accent)
...
**ProfileResolver** — Centralized batched profile fetching. 6-hour staleness threshold (1h for no-picture profiles). 15s in-flight guard prevents duplicate requests. Default scroll mode: 3 indexer relays. Profile screen fanout: up to 4 indexer relays via `requestWithFanout()`. `filterUnresolved()` uses staleness-aware `UserDao.getFreshPubkeys()` (checks existence AND updated_at > threshold in epoch seconds) — pre-filters at CardHydrator call sites to skip orchestration for already-resolved profiles.
...
Auth spam suppression: `authFailedRelays` set, warning logged once then suppressed, cleared on reconnect. Persistent replay guard: requires `PERSISTENT` purpose before replaying subs (blocks OUTBOX-only relays from 31+ unwanted persistent subs). `connectAndAwait()` checks `blockedUrls` before connecting (matches `connect()` behavior). Per-relay REQ queue: tracks active one-shot subs per relay URL (cap 10), queues overflow, flushes on EOSE/CLOSE via `sendOneShotToRelay()`. **Per-relay rate limiter (corrective):** token bucket (5 tokens/sec burst, `getAndUpdate` pre-decrement check — fixed off-by-one that reduced capacity from 5 to 4) gates all one-shot REQs via `canSendToRelay()`; token exhaustion logged as "Rate limit: token exhausted"; on CLOSED "rate-limited" message, 30s cooldown — all REQs dropped (not queued) during cooldown. Idle connection eviction: OUTBOX-only connections evict at 30s idle, BROWSE at 60s (PERSISTENT never evicted). Steady-state cap: 30s after startup, proactive sweep evicts idle non-PERSISTENT connections above cap of 10. **fetchEventById** is `suspend` — hints-first strategy: connectAndAwait hint relays (2s timeout), send REQ only to hints, NO broadcast fallback. No hints: capped fallback at 3 random non-indexer relays. `eventFetchInFlight` 30s TTL dedup.
...
- `isAtTop` false + no leading new → APPEND pagination items at bottom (no rebuild of existing items — data-only refreshes deferred to next MERGE when user scrolls back to top)
...
**FeedHydrationController** — Single zone-aware scroll state machine with three-layer engagement pipeline. **5-state machine:** WARM_CATCHUP (profiles for visible + warm zone, indexer relays, 3s timeout gate), SLOW_SCROLL (max 4 profiles + 2 refs per pass sorted by viewport proximity, indexer-only, engagement freshness pre-check for warm zone), IDLE (full fan-out for new items + deferred fan-out for scroll items, engagement freshness check for visible+warm zone, background backfill drip), FAST_SCROLL (total blackout, cancel all jobs), **REST (absolute rest — zero discretionary work, no hydration/backfill/engagement, entered 1s after IDLE, minimum 3s dwell for non-user exits, user scroll exits immediately)**. **Hard dwell lock (corrective):** 200ms cooldown enforced inside `transitionTo()` chokepoint — blocks ALL callers (velocity, timer, timeout) within dwell window. User gestures (`isUserGesture=true`) and `reset()` bypass. **Low-pass velocity filter:** `smoothedVelocity = 0.3f * raw + 0.7f * previous` — all state decisions in `nextState()` use smoothed signal, preventing noisy per-frame spikes from triggering oscillation. **Per-handler dedup:** profileJob/refJob active-job guards prevent overlapping hydration launches — each handler checks `?.isActive` before launching. No separate registry needed (previous ConcurrentHashMap-keyed registry was dead code — slice key changed every frame during scroll, removed in corrective sprint). **Three-layer engagement:** Layer 1 (background backfill) — slow drip of 15 events/2.5s covering entire feed, starts 5s after launch, paused during REST; Layer 2 (warm zone pre-check) — freshness check via Room `event_stats.updated_at`, max 5 stale items fetched per pass; Layer 3 (hot zone read-only) — visible items read from Room only, zero network calls. **Profile fan-out deferral:** SLOW_SCROLL/WARM_CATCHUP use `hydrateProfiles(fanOut=false)` (indexer only); source/hint relay fan-out batched to IDLE via `CardHydrator.fanOutProfiles()`. **Per-frame caps:** SLOW_SCROLL limited to 4 profiles + 2 refs sorted by viewport center proximity. Velocity detection via 6-frame sliding window with hysteresis (enter FAST >2500px/s, exit <1200px/s). Constructed in FeedViewModel with viewModelScope. Fed every frame from FeedScreen snapshotFlow (uses `rememberUpdatedState` for events). `reset()` starts catchup timeout + cancels backfill + clears in-flight maps.

**CardHydrator** — Stateless card hydration worker with two-phase API and two short-circuit guards. `hydrateProfiles(events, fanOut)` (Phase 1): pre-filters candidate pubkeys against Room via `ProfileResolver.filterUnresolved()` backed by `UserDao.getExistingPubkeys()` — pubkeys already cached are dropped before any coroutine launch, log line, or relay REQ; if all pubkeys cached, returns immediately. Resolves author profiles (kind 0), repost original-author profiles (NIP-18 p-tag); when `fanOut=true` also does nprofile relay hints + source relay fetching — fires instantly with no delay. `fanOutProfiles(events)`: deferred fan-out for source relay + hint relay profile fetches (called from IDLE for items hydrated with `fanOut=false` during scroll). `hydrateRefs(events)` (Phase 2): returns immediately when extracted ref set is empty (short-circuit — eliminates "20 cards → 0 refs" waste). When refs exist: referenced events for reposts (kind 6 e-tag with relay hint extraction via `extractRepostTargetRelay`) and quotes (nostr:nevent/note), referenced event author profiles, video thumbnail prefetch (max 3 per batch) — includes 1500ms delay for relay responses. No internal state tracking — FeedHydrationController decides what items to hydrate and when. Note: @Singleton is called from multiple scopes concurrently (feed + profile screens) — never use shared mutable collections.
...
User scrolls → snapshotFlow(scrollOffset + isScrolling + visibleIds) → every frame
    ...
    → State machine decides:
        WARM_CATCHUP: CardHydrator.hydrateVisibleCards(visible + warmZone) — profiles only
...
- Infinite scroll: loadMore() at 50% scroll position with 1s timestamp cooldown, fetchOlderEvents with `until` filter, isLoadingMore StateFlow, CircularProgressIndicator footer, displayLimit grows by 50 per page (capped at 1000). Monotonic `paginationCursor: Long` field (starts at MAX_VALUE, only advances backward, reset on feed switch) — `minOf(visibleOldest, paginationCursor)` prevents cursor jumping forward during MERGE. Uses `minOfOrNull { it.createdAt }` instead of `lastOrNull()` because visible list is not sorted after trickle APPENDs
...
- OpenGraph link preview cards (OgFetcher: browser UA, direct GET, og: → twitter: → HTML title fallback, ConcurrentHashMap cache, Html.fromHtml entity decoding, relative URL resolution, 4:3 aspect ratio container with ContentScale.Fit — no cropping, imageLoadFailed graceful collapse)
...
- NIP-57 zap flow: kind-9734 zap request → LNURL-pay → BOLT-11 invoice → NWC (NIP-47) payment. ZapRepository orchestrates, NwcManager handles NIP-04 encrypted WebSocket. LNURL metadata cached 5min, NWC WS pre-warmed during bolt11 fetch. Kind-9734 persisted to Room on success (drives zappedEventIds + sats persistence). ZapButton: loading spinner → scale bounce + white→amber flash on success, snackbar on failure.
- Action bar: ↩ reply, ⚡ zap (default 21 sats, long-press picker), 🔄 repost, ❤️ reaction, 🔗 share
...
| 200ms hard dwell lock (corrective) | Dwell lock enforced inside transitionTo() chokepoint — ALL callers gated (velocity, timer, timeout). User gestures bypass. Previous implementation only checked in nextState(), letting timer-based transitions bypass. |
| Low-pass velocity filter (corrective) | smoothedVelocity = 0.3*raw + 0.7*previous — state decisions use smoothed signal. Eliminates noisy per-frame oscillation during fling deceleration. Transitions dropped 75% (294→72 over 3min session). |
...
| displayLimit cap 300 (raised to 1000) | Original 300 prevented unbounded growth (50→1300+) causing 93MB GC → process death. Raised to 1000 with monotonic cursor in P_PAGINATION — OOM risk is in Coil bitmaps not FeedRow data |
...
| FeedStateReducer data-aware dedup | ID-only dedup at top (MERGE path refreshes data in-place when IDs unchanged). APPEND path: no-rebuild — skips entirely when no new items (data-only refreshes deferred to MERGE on scroll-to-top) |
...
| NIP-19 failure cache | Nip19FailureCache negative ConcurrentHashMap cache (MAX_SIZE 10000, soft single-entry eviction on overflow) at CardHydrator + NoteCard decode sites — prevents repeated parsing of bad bech32 data. Known-bad strings persist across session. |
...
| CardHydrator Phase1 Room pre-filter | hydrateProfiles() pre-filters candidate pubkeys against Room via ProfileResolver.filterUnresolved() + UserDao.getFreshPubkeys() (staleness-aware: checks existence AND updated_at > 6h threshold) — drops cached pubkeys before any coroutine/log/REQ. Eliminated 98% of Phase1 orchestration waste (959→19 calls on device) and 100% of "all fresh, skipping" (773→0) |
| CardHydrator Phase2 empty-ref short-circuit | hydrateRefs() returns immediately when extracted ref set is empty — skips Room lookups, relay fetches, 1500ms delay, author resolution, thumbnail prefetch. Eliminated 389 "20 cards → 0 refs" calls per session |
| fetchEventById hints-first | Made suspend + connectAndAwait hint relays (2s timeout) + send REQ only to hints. No broadcast fallback. No hints: 3 random non-indexer relays. NoteActionsViewModel restructured (synchronized block releases before suspend). Eliminated 100% wide broadcasts, token exhaustion -76.7% |
| ProfileResolver staleness pre-filter | UserDao.getExistingPubkeys → getFreshPubkeys (checks existence AND updated_at > threshold in epoch seconds). Pubkeys that exist but are stale no longer pass the filter as "fresh" |
| displayLimit cap 300→1000 + monotonic cursor | Raised OOM guard from 300 to 1000 (FeedRow data is ~2MB at 1000, OOM risk is in Coil bitmaps not list data). Added paginationCursor: Long (only advances backward, reset on feed switch). loadMore uses minOf(visibleOldest, paginationCursor) so cursor can't jump forward during MERGE. Uses minOfOrNull{createdAt} instead of lastOrNull() — visible list is not sorted after trickle APPENDs |
| APPEND no-rebuild | FeedStateReducer APPEND path no longer rebuilds `refreshed` list from Room data on every emission. When no new items found, returns cur unchanged (zero allocations, zero Compose diff). Data-only refreshes (engagement, profiles) deferred to MERGE on scroll-to-top. Eliminated 550+ item list rebuild on every Room re-emission while scrolled down |
| Coil 64MB memory cache cap | ImageLoader.Builder.memoryCache with 64MB hard cap — was unbounded (25% of app heap default). Limits bitmap accumulation in LRU cache during deep scroll sessions |
| Coil explicit size resolvers | rememberSizedImageRequest / rememberAvatarImageRequest / rememberFullWidthImageRequest helpers pass explicit pixel dimensions to Coil ImageRequest.Builder.size() — decodes bitmaps at display resolution instead of full source resolution. Applied to all AsyncImage/SubcomposeAsyncImage in feed (MediaImage, AvatarImage, EmbeddedQuoteCard, ArticleCard, LinkPreviewCard, VideoThumbnailBox), notifications, search, profiles, and nav bar |
| Coil crossfade | ImageLoader.Builder.crossfade(true) — smooth fade-in on image load, masks decode latency |
| NIP-19 negative cache soft eviction | Nip19FailureCache MAX_SIZE 500→10000, replaced failures.clear() with single-entry eviction on overflow — known-bad strings persist across session. Was wiping entire cache at 500, causing 936 re-parses of same invalid nprofile in 21min. Parse errors/min 44→0 (-100%) |
| NoteCard nostrRefs remember() wrapper | Wrapped NOSTR_URI_REGEX + decodeNostrRef extraction in remember(effectiveContent, qTagHints) — prevents re-parsing on every recomposition. Eliminated cascade recompositions downstream of nostrRefs (link previews, embedded quotes). Phase1/Phase2/min 55.7→18.2 (-67% bonus) |
| WARM_CATCHUP warm zone profiles | handleWarmCatchup() expanded from lastVisibleItems-only to (lastVisibleItems + computeWarmZone()).distinctBy{id}. Profiles for 15 items ahead now resolve as soon as fling deceleration begins (WARM_CATCHUP entry) — SLOW_SCROLL profile hydration never fired during real scroll because profileJob from WARM_CATCHUP was still active at transition (25ms dwell). Eliminates IdentIcon fallback during active browsing |
...
| April 9 (bug fix) | **Bug B: Follower/following counts + bio centering.** Two compounding bugs caused the stats row (Following/Followers counts) to be invisible on UserProfileScreen and unreliable on ProfileScreen. (1) UI gate `if (followingCount != null \|\| followerCount != null)` in UserProfileScreen hid the entire row when both counts were null — ProfileScreen had no such gate. Fix: removed gate, always render stats row with "—" fallback (StatLabel composable, matching ProfileScreen pattern). (2) Connection timing: `sendCount("wss://antiprimal.net")` and `fetchFollowingCount()` silently returned null when target relays weren't in the connections map. On navigation, antiprimal.net often evicted by 60s idle timer or blocked by 13-connection cap (all slots PERSISTENT). Fix: `connectAndAwait` before both count calls with `forceEvict=true` — new `forceEvictMostIdle()` method evicts the most idle OUTBOX connection (regardless of idle threshold) when normal eviction fails at cap. Applied to both ProfileViewModel and UserProfileViewModel. `fetchFollowingCount` in RelayPool.kt also gets internal `connectAndAwait(indexerUrls, forceEvict=true)` so both callers benefit. Bio centering: added `textAlign` parameter to `NostrRichText` composable (default null, no change for existing callers), used `TextAlign.Center` for bio on both ProfileScreen and UserProfileScreen. Visual validation on physical device (GrapheneOS): both screens show follower counts, bio centered. |
| April 9 (device perf — parser CPU burn) | **P_NEGATIVE_CACHE_FIX — physical device validated (GrapheneOS).** Post-P_HOT_2 21-min session (PID 24660) revealed dominant CPU burn from NIP-19 parser: 936 parse failures (44/min) on same invalid `nprofile1qqs24yz8xftq8kkdf7q5yzf4v7t` (invalid checksum), each allocating String + Bech32 decode + throw + log. Two bugs in existing code: (1) Nip19FailureCache self-destruct eviction — `failures.clear()` at MAX_SIZE=500 wiped entire cache including hot entries, forcing re-parse of known-bad strings on every cache refill cycle. Fix: MAX_SIZE 500→10000, replaced `clear()` with single-entry soft eviction. (2) NoteCard.kt:315 nostrRefs extraction had no `remember()` wrapper (unlike 2 other call sites at 1644 and 1770) — every recomposition re-ran regex + decodeNostrRef, producing new list references that triggered cascade recompositions downstream (link previews, embedded quotes, hydration re-triggers). Fix: wrapped in `remember(effectiveContent, qTagHints)`. Results (3.95min, PID 13191): NIP-19 errors/min 44→0 (-100%), Phase1/Phase2/min 55.7→18.2 (-67% bonus from cascade elimination), frame drops/min 2.1→1.8 (-14%), GC profile shifted from parser churn (70-100MB AllocSpace, zero LOS) to normal app memory (30-65MB AllocSpace, healthy LOS). Regression checks clean: Phase2 0-refs=0, fetchEventById wide broadcasts=0. Artifact: validation_p_neg_cache_20260409_133643.log. |
| April 9 (device perf) | **P_HOT_2 Coil bitmap pressure fix — physical device validated (GrapheneOS).** Baseline from April 9 ~00:02 (PID 8954, relay.ditto.pub NOTES_ONLY, 2.5min): 14 GC events, max 74MB freed, 72 frame drops, largest 134 frames, Phase1/Phase2 72. Root cause: Coil 3.0.4 ImageLoader had no memory cache cap (default 25% of heap) and every AsyncImage used raw String URLs as model — Coil decoded full-resolution bitmaps for views rendering at a fraction of the size. **Fix:** (1) ImageLoader.Builder.memoryCache with 64MB hard cap + crossfade(true). (2) Three composable helpers in ui/common/ImageRequestHelpers.kt: `rememberSizedImageRequest(url, widthPx, heightPx)`, `rememberAvatarImageRequest(url, sizeDp)`, `rememberFullWidthImageRequest(url, aspectRatio)` — all pass explicit pixel dimensions to ImageRequest.Builder.size(). (3) Applied to all feed-critical call sites: MediaImage (SubcomposeAsyncImage), AvatarImage wrapper (added sizeDp parameter), EmbeddedQuoteCard single+grid images, ArticleCard banner+avatar, VideoThumbnailBox poster, LinkPreviewCard OG thumbnail. (4) Feed-adjacent: NotificationEventRow avatar, SearchScreen avatar, ArticleReaderScreen banner. (5) Profile screens: ProfileScreen/UserProfileScreen banners (screen-width × 200dp) and avatars (85dp), AppNavigation bar avatar (dynamic 20-32dp). Intentionally skipped: FullScreenImageDialog (needs full res), YouTube thumbnail (480×360 at source), InlineAutoPlayVideo poster (server-scaled). AvatarImage callers with non-default sizes updated: 16dp reposter, 24dp quote/address/thread. Rate-normalized results (4m10s session, 800+ items via 16 loadMore): GC events/min 5.6→1.9 (-66%), GC MB/min 308→116 (-62%), frame drops/min 28.8→2.9 (-90%). LOS column confirmed bitmaps averaging 17-43KB (correctly sized). Artifact: validation_p_hot_2_20260409_084920.log. |
| April 8 (device perf) | **Device Performance Sprint — physical device validated (GrapheneOS).** Emulator corrective sprint passed but physical device under sustained relay traffic exposed catastrophic orchestration waste: 959 Phase1/Phase2 calls (80.6% doing zero work), 773 "all fresh, skipping", 115 frame drops, 44 resource leaks, 323 token exhaustion events over 3.6min. Structural correctness from the corrective sprint held (zero sub-200ms violations, zero low-velocity FAST_SCROLL, dwell lock actively blocking). **P1 CardHydrator short-circuits:** Two surgical guards — (1) hydrateRefs() returns immediately when extracted ref set is empty (eliminated 389 "20 cards → 0 refs" calls), (2) hydrateProfiles() pre-filters candidate pubkeys against Room via ProfileResolver.filterUnresolved() + UserDao.getExistingPubkeys() (drops cached pubkeys before any coroutine/log/REQ). Results: Phase1/Phase2 959→19 (-98%), "all fresh" 773→0 (-100%), frame drops 115→0, resource leaks 44→1, token exhaustion 323→3 (-99.1%). Artifact: validation_p1_shortcircuit_20260408_184202.log. **P_HOT_1 hints-first event fetch + staleness pre-filter:** (1) fetchEventById made suspend, connectAndAwait hint relays (2s), send REQ only to hints — no broadcast fallback. No hints: 3 random non-indexer relays. (2) ProfileResolver.filterUnresolved switched from existence-only getExistingPubkeys to staleness-aware getFreshPubkeys (updated_at > 6h threshold in epoch seconds). Results: token exhaustion -76.7%, relay.ditto.pub exhaustion 172→0, fetchEventById wide broadcasts 67→0, frame drops 49→14. Artifact: validation_p_hot_1_fanout_20260408_191247.log. **P_PAGINATION structural pagination fix:** Feed query was sliding window (ORDER BY created_at DESC LIMIT 300) — new real-time events pushed older content out of LIMIT. Users hit empty feed at 300 items. Three fixes: (1) displayLimit cap 300→1000, (2) monotonic paginationCursor (only advances backward, minOf with visibleOldest), (3) minOfOrNull{createdAt} instead of lastOrNull() (visible list unsorted after trickle APPENDs — lastOrNull read recent timestamps causing duplicate-guard deadlock after first loadMore). (4) FeedStateReducer APPEND no-rebuild: skip refreshed list rebuild on data-only Room emissions, defer to MERGE. Results: loadMore fires 1→17, max feed depth 104→907, cursor strictly monotonic, content keeps appearing through 900+ items. Remaining jank at 500+ from Coil bitmap pressure (P_HOT_2 next sprint). Artifact: validation_pagination_v3_20260408_200336.log. |
| April 8 (corrective) | **Corrective Sprint — human-in-the-loop validated.** Previous April 8 PM sprint self-reported success via scripted `adb shell input swipe` but real hand-driven testing showed 250 transitions (baseline 249), 12 at v=0px/s, zero single-flight hits, 66 resource leaks (up from 32). This sprint re-does P1/P2/P5/P3 with human-in-the-loop validation per VALIDATION_PROTOCOL.md. **P1:** Dwell lock moved INTO transitionTo() as first check (was only in nextState(), so timer-based transitions bypassed it). Low-pass velocity filter (smoothedVelocity = 0.3*raw + 0.7*previous) replaces raw velocity for ALL state decisions — eliminates noisy per-frame oscillation during fling deceleration. Results: 72 transitions over 3min session (was 294 without smoothing), zero sub-200ms violations, zero low-velocity FAST_SCROLL, 133 dwell blocks. Artifact: validation_p1_dwell_velocity_smooth_20260408_161218.log. **P2:** Investigation found single-flight ConcurrentHashMap registry never fired (slice key changed every frame during scroll). Removed dead infrastructure — real dedup is profileJob/refJob active-job guards at call sites. Results: 43 Phase1/Phase2 (was 520 broken), 8 "all fresh" (was 384 broken). Artifact: validation_p2_cleanup_20260408_162648.log. **P5:** Investigation of all closeable resource paths (OgFetcher, ZapRepository, MediaPreconnect, VideoThumbnailCache, Migrations Cursor, RelayConnection WebSocket, NwcManager WebSocket) found zero app-side leaks. The April 8 regression from 32→66 leaks was downstream of state machine hyperactivity amplifying Coil image decode pressure → HWUI BitmapFactory finalizer warnings. P1+P2 fixes reduced transitions 75%, which normalized leaks to 6 under intentional image load (all HWUI framework internals). GC max 16MB (was 70MB+), zero frame drops (was 61). No code changes. Artifacts: validation_p2_cleanup_20260408_162648.log (2 leaks), validation_p5_media_confirm_20260408_165535.log (6 leaks). **P3:** Token bucket off-by-one fix (`updateAndGet` → `getAndUpdate` — pre-decrement check restores full 5-token capacity) + token exhaustion logging added. Zero rate-limit warnings from relays, 12 token exhaustion events on 4 relays. Artifact: validation_p3_rate_limit_20260408_170104.log. |
| April 8 (PM) | **Structural State Machine Rewrite + Sync Foundation:** P0+P1 REST state + hard dwell lock: 5-state machine (WARM_CATCHUP/SLOW_SCROLL/IDLE/FAST_SCROLL/**REST**). REST = absolute zero discretionary work, entered 1s after IDLE, 3s minimum dwell for non-user exits, user scroll exits immediately. Hard dwell lock (200ms) prevents velocity-driven flapping during fling deceleration — user gestures bypass. P2 single-flight slice execution: ConcurrentHashMap-keyed in-flight registry for profile+ref hydration eliminates concurrent hydration of same window. P3 per-relay rate limiter: token bucket (5 tokens/sec) + 30s cooldown on CLOSED "rate-limited". P4 background sync foundation (v17→v18): sync_state table, first_seen_at on events, SyncStateEntity+Dao, EventProcessor sets firstSeenAt on insert, RelayPool updates sync_state on persistent sub EOSE, BackgroundSyncWorker stub (heartbeat only), WorkManager scheduling (30min, network+battery). **Results vs baseline (748/639/249/14/126/7):** Phase1/Phase2: 12 (98.4% reduction), "all fresh": 0, transitions: 31 (87.5% reduction), frame drops: 0, resource leaks: 0, rate-limit warnings: 0. Previous "DO NOT TOUCH state machine STRUCTURE" guardrail is now lifted. **Note: self-reported metrics from scripted validation proved unreliable — see corrective sprint above.** |
...
| April 10 (UX polish) | **UX polish sprint (5 items + zap flow fix).** (1) **App-wide snackbar:** SnackbarHostState + CompositionLocal `LocalShowSnackbar` in AppNavigation, used for copy npub, follow/unfollow, zap failure feedback. (2) **Share button:** NoteCard + ArticleCard + ArticleReaderScreen action bars get share icon → Android Intent.ACTION_SEND with njump.me link. (3) **Zap flow fix (4 bugs):** Root cause: NWC wallet returns `"error": null` on success — JsonNull is a JsonElement, so `resp["error"] != null` was true, entered error branch, called `.jsonObject` on JsonNull → crash. CompletableDeferred never completed in catch block (30s timeout). ViewModel ignored `Result` — optimistic UI lied. Fix: JsonNull type check, deferred completion in catch, Result-driven UI (loading spinner → success animation / failure snackbar). **Zap performance:** LNURL metadata cache (5min TTL saves 400-850ms on repeat zaps), parallel NWC WebSocket warmup (connects during bolt11 HTTP fetch, saves ~400ms), human-readable error messages (PAYMENT_FAILED/INSUFFICIENT_BALANCE/QUOTA_EXCEEDED/NOT_FOUND). Timeout 30s→15s. **Zap persistence:** Kind-9734 zap request inserted into Room on success → `zappedEventIds` query finds it → amber state survives app restart. `event_stats.zap_total_sats` incremented for target event → sats display persists. **Zap animation:** Scale bounce (1.0→1.8→spring back) + white→amber color flash (400ms tween). (4) **Settings fake doors:** 7 settings stubs marked `enabled = false` with alpha 0.4 + "Coming soon" subtitle. (5) **Thread OP accent:** Cyan left border on focused note in ThreadScreen. Default zap amount 21 sats across all screens. Zap preset amounts: 21/100/500/1k/5k/10k/50k/100k. ZapRepository returns `Result<Event>` (was `Result<Unit>`) so ViewModel can persist the signed event. |
| April 10 | **OG card aspect ratio fix:** LinkPreviewCard image container changed from fixed height (160dp) + ContentScale.Crop to aspectRatio(4f/3f) + ContentScale.Fit — standard OG images (1200×630) fully visible with slight letterboxing instead of top/bottom cropping. Image request heightPx computed from screen width × 3/4 ratio. Loading placeholder uses matching aspectRatio. **WARM_CATCHUP warm zone profile pre-fetch:** handleWarmCatchup() expanded from lastVisibleItems-only to (lastVisibleItems + computeWarmZone()).distinctBy{id}. Root cause: SLOW_SCROLL profile hydration never fired during real scrolling — profileJob from WARM_CATCHUP was still active when state transitioned (25ms dwell), so SLOW_SCROLL's profileJob?.isActive guard returned early. Profiles for 15 items ahead now start resolving at fling deceleration (WARM_CATCHUP entry). Fan-out stays disabled (IDLE-only). Fewer IdentIcon fallbacks during active browsing. **Bug 1 (Conversations tab media):** Investigated — ThreadParentCard is intentionally compact (text-only, no media). Deferred media upgrade to future sprint. **P_SUB_TARGETING:** Persistent subscription replay storm fix — 4 feed subscription `registerPersistentSub` calls (posts, media, longform in subscribeAfterConnect + follows in connectForAuthors) now pass `targetRelayUrl` so replay only fires on the target relay's reconnect instead of broadcasting to all relays. Notifications persistent sub intentionally left untargeted (global). Null crash guard on `fetchProfilesFromHints`: `normalizeRelayUrl()` can return null, wrapped in `?.let` before ConcurrentHashMap lookup. **P_SINGLE_RELAY_FANOUT:** Source-relay fan-out skip for single-relay feeds — `FeedHydrationController.feedRelayUrl` set from `FeedType.SingleRelay.url` on feed switch, threaded through `launchProfileHydration`/`startIdleEngagement` as `excludeSourceRelay` to CardHydrator. Source relays filtered with `.filter { it != excludeSourceRelay }`, hint relays unaffected. **Immersive scroll content jerk fix:** Root cause: two animated padding collapses when bars hid — `contentTopPadding` in AppNavigation (topBarHeight+statusBarHeight→0) and tab row height in FeedScreen (48dp→0) physically shifted content upward. Fix: replaced layout padding with LazyColumn `contentPadding = PaddingValues(top = totalTopPadding)` (standard collapsing toolbar pattern). Content area starts at y=0, top bar overlays via offset (unchanged), tab row changed from Column height-collapse to Box overlay with offset animation (250ms, FastOutSlowInEasing, same timing as top bar). When bars hide, LazyColumn viewport expands upward but scroll position preserved — no jerk. Non-feed screens (Search, Notifications) get constant padding. **Notification blue dot:** `NotificationsViewModel` tracks "last seen" timestamp via SharedPreferences (`notif_state/last_seen_ts`). DAO flow emission compares top item's `createdAt` to stored timestamp → `hasNewNotifications: StateFlow<Boolean>`. Blue dot (6dp cyan circle) rendered on bottom nav bell icon (tab 2), same pattern as feed tab's new-post dot. `markSeen()` called on tab switch, stores current top notification's timestamp. NotificationsScreen LazyColumn gets `contentPadding = PaddingValues(top = staticTopPadding)` for immersive scroll support. |
...
2. Settings tab — shell exists (Relays + Logout functional, 7 stubs marked "Coming soon"), needs: Keys (nsec export), Safety (mute/block UI), Wallet (NWC URI management, currently paste-only dialog), Cache (clear Room), version/source link. Drafts/Social Graph/Custom Emojis/Console are future.
3. Image pinch-zoom — FullScreenImageDialog exists but has no zoom/pan; needs transformable gestures (scale + offset)
...
snapshotFlow { Triple(scrollOffset, isScrollInProgress, visibleIds) }
    ...
    → state machine: WARM_CATCHUP (profiles for visible+warm zone), SLOW_SCROLL (warm zone),
