...
**Last updated:** May 7, 2026 (P14: adaptive arrival coalescing, per-event statsFlow, streaming JSON decode in EventProcessor, trust+RTT relay ranking, auto-paging, V3 binary snapshot. P13: per-author FeedRow cache key, sampled signal trigger, control-plane channel batching, merge-sort `_events`, viewport-driven warm-zone hydration via CardHydrator.)
...
- **TimelineConsumer** — sole feed-state primitive (FeedViewModel, ProfileViewModel, UserProfileViewModel all delegate to instances). Owns `_events: MutableStateFlow<List<NostrEvent>>`, `_pendingNew`, `_isAtTop`, `_isLoading`. Subscribes via `TimelineService.subscribeTimeline` and routes incoming relay batches through `mergeSorted(current, newEvents)` — dedup via HashSet, sort only the novel events, two-pointer merge into the existing sorted list. Append-mode growth: live tail-in at top when `_isAtTop`, blue-dot pending buffer when scrolled, `loadMore()` extends at bottom. **Adaptive arrival coalescing**: tap callbacks enqueue batches into an unbounded Channel; a drainer applies them in coalesced windows that adapt 100ms–500ms based on observed rate (Following stays at min, Ditto-class firehose stretches to max). `feedRows: StateFlow<List<FeedRow>>` derived purely from `_events × _contentFilter` — no signal triggers. Profile and stats reactivity are per-card: EventCard observes `profileFlow(model.pubkey)` and `statsFlow(model.engagementId)` directly. New posts tail in within one window; profile/count updates re-compose only the affected cards.
- **CardHydrator** (data/relay) — viewport-driven warm-zone prefetcher. `hydrateVisibleCards(events)` resolves profiles, quoted-note bodies, OG previews, and image dimensions for a slice of the feed. Wired from `FeedViewModel.init` via `combine(consumer.events, _viewportFirstVisible).debounce(300)` running `collectLatest`, so a fast scroll cancels in-flight hydration and re-fires for the new zone. Warm zone = `WARM_ZONE_ABOVE=10` rows above + `WARM_ZONE_BELOW=30` below `firstVisibleIndex`. By the time a row enters the hot zone (visible viewport), its skeleton data is already in MES — no avatar pop-ins, no dimension shifts. CardHydrator de-duplicates internally so re-firing on an overlapping zone is cheap. Replaces the pre-P8d `FeedWindow`/`FeedWindowLoader` zone-aware hydration that was deleted but never re-attached
...
33b. **`connectAndAwait` waits for pending connections** — when all requested URLs already exist in `connections` (from a prior fire-and-forget `connect()` call), polls for at least one to reach `isConnected` before returning. Prevents race where `sendToRelay` fails because WebSocket handshake is still in progress
33c. **`sendToRelay` normalizes URLs** — callers may pass raw relay URLs; `sendToRelay` normalizes before looking up in `connections` map to prevent key mismatches
33d. **Bootstrap Step 5 uses `connectAndAwait`, not `connect()`** — `signalFeedConnectionsReady()` must fire AFTER connections are actually ready, not after fire-and-forget starts. Prevents NotifVM and FeedVM from sending REQs to unconnected WebSockets
...
39b. **Subscription normalizes URLs in SubState** — `urls.mapNotNull { normalizeRelayUrl(it) }.toSet()` ensures `eosedRelays` (populated with normalized URLs from EventProcessor) matches `SubState.urls` for correct `allEosed` checks and watchdog skips
...
45. **Warm-zone hydration is viewport-driven** — `FeedViewModel.init` runs `combine(consumer.events, _viewportFirstVisible).debounce(300)` → `cardHydrator.hydrateVisibleCards(warmZone)` via `collectLatest`. Warm zone = `firstVisibleIndex - 10` to `firstVisibleIndex + 30`. Skeleton (profiles, refs, OG, image dims) lands BEFORE the row enters the hot zone — no avatar pop-ins. CardHydrator de-duplicates internally; re-firing on overlapping zones is cheap. NEVER re-introduce per-card on-mount fetches without coordinating with this pass — duplicate work + cache thrash
46. **TimelineConsumer.feedRows has no signal triggers** — derived purely from `_events × _contentFilter`. Profile reactivity is per-card (EventCard observes `profileFlow(model.pubkey)`). Stats reactivity is per-card (EventCard observes `MES.statsFlow(model.engagementId)` via FeedViewModel.statsFlow). Adding any list-level signal trigger back reintroduces full-list recompute on every kind-0/7/9735 arrival anywhere
47. **`mergeSorted` instead of full re-sort** — TimelineConsumer.handleNewEvents/addCachedEvents/flushPending/loadMore use the in-place merge helper, NOT `(new + old).distinctBy.sortedWith`. The full-sort path was an O(N log N) per-batch hot loop with 30+ sorts during a Following warm-up
48. **FeedRow cache key is per-author + per-event** — `MES.toFeedRow` uses `profileUpdatedAt[event.pubkey] + statsUpdatedAt[statsId]` as the cache key, NOT the global `_profileSignal.value` / `_statsSignal.value`. Profile X arriving only invalidates rows by X; other rows stay cached. Don't regress to global version keys
...
**Warm-zone hydration:** `grep "CardHydrator:"` — verify per-zone fetch volumes and dedup. `grep "Phase1 profiles"` for profile batch sizes
...
**MES feed query:** `eventsByAuthors(authors, kinds, limit=300)` / `recentEvents(kinds, limit=300)` / `eventsByRelay(url, kinds, limit=300)` scan `recentByCreatedAt`, filter by kind/pubkey/relay, return raw `NostrEvent` lists for cache hydration. Called by `FeedViewModel.loadCachedEvents()` once per feed switch — not continuously. `feedRowsByIds(ids)` converts to `FeedRow` for UI; called by `TimelineConsumer.feedRows` and by `CardHydrator` for warm-zone work
