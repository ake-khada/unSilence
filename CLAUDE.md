...
- **Publish is outbox-targeted** — `publish(eventJson, targetRelays)` sends to the resolved relay set ONLY, not `connections.values`; the single-arg broadcast `publish(eventJson)` is gone. Reactions/reposts/profile → `engagementPublishRelays` (own write + target author read/inbox + seen + hints). **Zap requests are the INVERSE** — the kind-9734 `["relays",…]` tag (where the wallet publishes the kind-9735 receipt) uses `zapReceiptRelays` = target author **write** + own **read** + seen + hints, mirroring the engagement READ path (`resolveEngagementRelays`) so the receipt lands where the app fetches it. Using `engagementPublishRelays` for zaps put the receipt where we never read → sats count never updated (H20c follow-up)
...
- **NIP-36 sensitive-content gating consumes the `FeedRow`/`EventEntity.hasContentWarning` flag (upstream), NOT `EventModel.warnings`.** That flag is set by `effectiveContentWarning(kind, content, tags)` (shared top-level in EventProcessor) in **both `EventProcessor` and `Subscription`** (dual-path) — it ORs the **inner** event's `content-warning` for kind-6/16 embedded-JSON reposts (else a repost of sensitive content renders unflagged; wrapper reason wins). Snapshot restore recomputes 6/16 rows in both binary + text paths (migration). `ContentParser.warnings` is derived effective too but is NOT the gate
- **Embedded targets gate on the TARGET's own warning, not the wrapper's** — `EmbeddedSensitiveGate` (SHOW / BLUR=blur+tap-reveal / HIDE=compact placeholder, content never composed) wraps quoted notes (`QuoteCard`), reposted targets (`EmptyRepostBody`), and thread parents (`ThreadParentCard`); `EmbeddedArticleCard` self-gates via its own `EventCard` for quoted naddr/address long-form. `EventCard` takes `sensitiveMode` + `isSensitive` (NOT a `sensitiveBlur` boolean) and the mode MUST be threaded through **every** card surface — feed, profile, user-profile, thread, search, article-reader comments — not just feed (feed also drops HIDE rows via its filter; non-feed relies on the card-level HIDE placeholder, which preserves thread structure)
- **BLUR uses `Modifier.blur` — it can't blur an actively-playing video SurfaceView** (same limitation as the top-level card; do NOT alpha-hide the SurfaceView — breaks playback). HIDE is the strong guarantee. Main article-being-read is intentionally ungated (deliberate navigation) (fcf5d6bc)
...
- **Article body = native `MarkdownContent` (NO WebView).** kind-30023 body: `NativeMarkdownParser` (pure-Kotlin, `ParseLimits`-bounded) → `MdBlock` model → `ui/markdown/MarkdownContent` Compose blocks. Inline link/hashtag/mention styling has ONE source — `ui/feed/InlineSpans.kt` (`appendLinkSpan`/`appendHashtagSpan`/`appendMentionSpan`), shared by `InlineText`(Segment) and `MarkdownContent`(MdInline). Never fork a second inline renderer or re-add the WebView. Tables: equal-weight fit-to-viewport columns + `IntrinsicSize.Min` uniform rows (NOT char-width vector — overflowed). Body wrapped in one `SelectionContainer` (coexists with `LinkAnnotation` taps). `ARTICLE-PARSE-HEAVY` `Log.w` probe fires on `document.truncated`
