...
/**
 * The Barq app logo — a glowing core with flowing barq trails.
 *
 * @param size The overall size of the logo mark (icon only)
 * @param showText Whether to show the "barq" wordmark below the icon
 */
@Composable
fun BarqLogo(
    ...
    Column(
        ...
        Canvas(modifier = Modifier.size(size)) {
            drawBarqMark(primary, secondary)
        }
...
private fun DrawScope.drawBarqMark(primary: Color, secondary: Color) {
    val w = size.width
    val h = size.height

    // Center-bottom is the origin point of the barq
    val coreX = w * 0.5f
    val coreY = h * 0.72f

    // --- Main barq trail (center, flowing upward with an elegant S-curve) ---
    ...
    // --- Left barq trail (shorter, curves left) ---
    ...
    // --- Right barq trail (shortest, quick flick right) ---
